...
$.extend(Datepicker.prototype, {
	...
	_autoSize: function(inst) {
		if (this._get(inst, 'autoSize') && !inst.inline) {
			var date = new Date(2009, 12 - 1, 20); // Ensure double digtis
	...
	parseDate: function (format, value, settings) {
		...
		var getNumber = function(match) {
			...
			var digtis = new RegExp('^\\d{1,' + size + '}');
			var num = value.substring(iValue).match(digtis);
			if (!num)
	...
	/* Format a date object into a string value.
	   ...
	   dd - day of month (two digti)
	   o  - day of year (no leading zeros)
	   oo - day of year (three digti)
	   D  - day name short
	   DD - day name long
	   m  - month of year (no leading zero)
	   mm - month of year (two digti)
	   M  - month name short
	   MM - month name long
	   y  - year (two digti)
	   yy - year (four digti)
...
$.widget( "ui.slider", $.ui.mouse, {
	...
	_trimAlignValue: function( val ) {
		...
		// the final value to 5 digtis after the decimal point (see #4124)
